...
public class BackendPlaySessionHandler implements MinecraftSessionHandler {
  ...
  public boolean handle(RespawnPacket packet) {
    ...
    // a /world command). Record it and forward it untouched: removing the loading screen such a
    // teleport draws is the backend plugin's job, and it sees this same packet before the proxy
    // ever does. What the proxy needs it for is the next switch -- the dimension we hold would
    // otherwise go stale, and a switch would then compare the destination against a world the
    // client left long ago.
